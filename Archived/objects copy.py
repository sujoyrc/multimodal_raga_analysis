...
class Data:
    ...
    def load(self):
        '''
        Loads the data into X, y tuples from data file
        ...
        return ((X_train, y_train, train_ids, mask_train), (X_test, y_test, test_ids, mask_test), channels)
    ...
    def log_and_load(self, params):
        ...
        with wandb.init(
            ...
            ((X_train, y_train, train_ids, mask_train), (X_test, y_test, test_ids, mask_test), channels) = self.load()
            ...
            datasets = [[X_train, y_train, train_ids, mask_train], [X_test, y_test, test_ids, mask_test]]
            names = ['train', 'test']
            for name, data in zip(names, datasets):
                with data_art.new_file(name + '.npz', mode='wb') as file:
                    X_0 = data[0]
                    if data[3] is None or data[3].tolist() is None:
                        X_0_mask = None
                    else:
                        X_0_mask = np.append(data[0], data[3], axis=-1)
                    X_1 = [np.reshape(data[0][:, :, i], (-1, data[0].shape[-2], 1)) for i in range(data[0].shape[-1])]
                    np.savez(file, X_0=X_0, X_0_mask=X_0_mask, X_1=X_1, y=data[1], ids=data[2], mask=data[3])
        ...
        return ((X_train, y_train, train_ids, mask_train), (X_test, y_test, test_ids, mask_test))
...
class ModelInstance:
    ...
    def _build_model(self, input_shape):
        '''
        (used only for older params.json files (Expt7 and below))
        ...
        inputs = Input(shape=tuple(input_shape[1:]))     # input layer
        prev_layer = inputs

        # iterate through layer indices
        layers = [l for l in list(self.params['layers'].keys()) if l != 'output']
        layers.sort()
        layer_objs = {}     # dictionary of layers created in model
        for layer in layers:
            if 'conv' in list(self.params['layers'][layer].keys()) or 'pool' in list(self.params['layers'][layer].keys()):
                prev_layer = self._create_conv_layer(layer, prev_layer)
            elif 'flatten' in list(self.params['layers'][layer].keys()):
                prev_layer = self._create_flat_layer(layer, prev_layer)
            elif 'dense' in list(self.params['layers'][layer].keys()):
    ...
    def build_model(self, input_shape, run, split_inputs=None):
        '''
        ...
        Parameters
            input_shape (tuple): shape of input data
            split_inputs (bool): if True, will process each channel separately in the conv blocks and then merge everything in the flatten layer
        '''
    ...
    def train_model(self, data=None, split_inputs=None, run=None, log_evaluation=True, log_w_data=False):
        '''
        Train model

        Parameters:
            data (str): run name of data artifact stored
            split_inputs (bool): if True, each channel is processed separately until the layer with merge in it.
        ...
        #             if channel in self.params['mask_channels']:
        #                 new_X_train.append(np.append(X_train[:, :, channel_ind], mask_train, axis=-1))
        #                 new_X_test.append(np.append(X_test[:, :, channel_ind], mask_test, axis=-1))
        #             else:
        #                 new_X_train.append(X_train[:, :, channel_ind])
        #                 new_X_test.append(X_test[:, :, channel_ind])
        #         X_train = new_X_train
        #         X_test = new_X_test
        #     else:
        ...
        # else:
        #     if self.params['mask']:
        #         # append mask to the last channel
        #         pdb.set_trace()
        #         X_train = np.append(X_train, mask_train, axis=-1)
        #         X_test = np.append(X_test, mask_test, axis=-1)
        ...
        if log_evaluation:
            if log_w_data:
                valid_table = run.use_artifact(self.params['metadata_artifact'] + ':latest')
                valid_indices = valid_table.get('test_table').get_index()
                callback_values.append(WandbCallback(monitor="val_accuracy", log_evaluation=True, log_weights=True, log_gradients=True, training_data=(X_train, y_train), validation_data=(X_test, y_test), validation_indexes=valid_indices, log_evaluation_frequency=50))
            else:
    ...
    def log_train_model(self, data=None, sweep=True, split_inputs=None, log_train_metadata=False, log_evaluation=False, log_train_w_data=False):
        '''
        Logs training of model on wandb

        Parameters:
        data_name (tuples): ((X_train, y_train), (X_test, y_test))
        ...
        split_inputs (bool): if True, each channel will be processed separately
        '''
        ...
        with wandb.init(
            ...
            if data is not None:
                ...
                for data_art in self.params['data_artifact']:
                    ...
                    # choose the right version of X data
                    if self.params['mask']:
                        if not split_inputs:
                            X_key = 'X_0_mask'
                        else:
                            ...
                            X_test_temp = []
                            for channel_ind, channel in enumerate(self.channels):
                                # add mask to the channels listed in 'mask_channels'
                                if channel in self.params['mask_channels']:
                                    X_train_temp.append(np.append(train_data['X_1'][channel_ind], train_data['mask'], axis=-1))
                                    X_test_temp.append(np.append(test_data['X_1'][channel_ind], test_data['mask'], axis=-1))
                                else:
                                    X_train_temp.append(train_data['X_1'][channel_ind])
                                    X_test_temp.append(test_data['X_1'][channel_ind])
                    else:
                        if not split_inputs:
                            X_key = 'X_0'
                        else:
                    ...
                    if X_key is not None:
                        ...
                        X_train, y_train, train_ids, mask_train = train_data[X_key], train_data['y'], train_data['ids'], train_data['mask']
                        test_data = np.load(os.path.join(data_dir, 'test' + '.npz'), allow_pickle=True)
                        X_test, y_test, test_ids, mask_test = test_data[X_key], test_data['y'], test_data['ids'], test_data['mask']
                    else:
                        X_train, y_train, train_ids, mask_train = X_train_temp, train_data['y'], train_data['ids'], train_data['mask']
                        test_data = np.load(os.path.join(data_dir, 'test' + '.npz'), allow_pickle=True)
                        X_test, y_test, test_ids, mask_test = X_test_temp, test_data['y'], test_data['ids'], test_data['mask']
                    
                    # add the data as a class variable
                    self.data[data_art] = ((X_train, y_train, train_ids, mask_train), (X_test, y_test, test_ids, mask_test))
            
            # store shape of inputs
            if split_inputs:
                input_shape = [x.shape for x in self.data[0][0]]
            else:
                input_shape = {d_key: d[0][0].shape for d_key, d in self.data.items()}

            # build model
            self.build_model(input_shape, run=run, split_inputs=split_inputs)
            ...
            run = self.train_model(split_inputs=split_inputs, run=run, log_evaluation=log_train_metadata, log_w_data=log_train_w_data)
            ...
            train_loss, train_acc, test_loss, test_acc, train_preds, test_preds, train_ids, test_ids = self.evaluate(log=False, split_inputs=split_inputs)
    ...
    def evaluate(self, data=None, log=False, split_inputs=None, log_metadata=True):
        '''
        Evaluates model
        ...
        # if data is not None:
        #     ((X_train, y_train, _, _), (X_test, y_test, _, _)) = data
        # else:
        #     ((X_train, y_train, _, _), (X_test, y_test, _, _)) = self.data
        ...
        if not log:
            ...
            # log cm into wandb
            # train cm
            cm_train = wandb.plot.confusion_matrix(
                y_true=np.argmax(y_train, axis=1),
                preds = np.argmax(model.predict(X_train), axis=1),
                class_names = self.raga_labels
            )
            wandb.log({'Train_CM': cm_train})
            # test cm
            cm_test = wandb.plot.confusion_matrix(
                y_true=np.argmax(y_test, axis=1),
                preds = np.argmax(model.predict(X_test), axis=1),
                class_names = self.raga_labels
            )
            wandb.log({'Test_CM': cm_test})
    ...
    def log_and_evaluate(self, data=None, log_metadata=False): 
        ...
        with wandb.init(
            ...
            train_loss, train_acc, test_loss, test_acc, train_preds, test_preds, train_ids, test_ids = self.evaluate(log=False)
    ...
    def log_and_evaluate_class(self, data=None): 
        ...
        with wandb.init(
            ...
            if data is None:
                ...
                data_dir = dataset.download()
                train_data = np.load(os.path.join(data_dir, 'train' + '.npz'), allow_pickle=True)
                X_train, y_train, train_ids, mask_train = train_data['X'], train_data['y'], train_data['ids'], train_data['mask']
                test_data = np.load(os.path.join(data_dir, 'test' + '.npz'), allow_pickle=True)
                X_test, y_test, test_ids, mask_test = test_data['X'], test_data['y'], test_data['ids'], test_data['mask']
